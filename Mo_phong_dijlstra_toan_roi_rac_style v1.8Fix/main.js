...
window.onload = function () {
    ...
    // Add info to info field  Thêm dữ liệu vào table thông tin
    send_pathInfo = function (Testing) {
      path_info_field.innerHTML += " -> " + Testing + "<br>";
      path_info_field.scrollTop = 9999; // Scroll to last string in field  Tự động cuộn nội dung dọc xuống
    ...
    */
    ...
    var checkDistancesStep = function () {
        ...
        if(newDistance < distances[edge.to]) {
            ...
            // Thay đổi màu sắc cho các nút trước đó
    ...
    // Giải thích tóm tắt cách hoạt động của chương trình:
    // Là chương trình sẽ cho phép chúng ta tự tạo đồ thị bằng chuột và mình họa nó trực tiếp bằng hình ảnh
    // Giống như cách mà "Thầy An" dạy toán rời rạc đã minh họa bằng "bảng phấn"
    /* Cùng với lệnh setTimeout 
           Làm chậm lại quá trình chạy code với đơn vị thời gian là giây
            Nghĩa là từng hàm sẽ chạy lần lượt
            Hàm A chạy xong mới tới hàm B chạy
            Phục vụ cho việc xuất thông tin "Giảng dạy"
           */

                // Cần giải thích rõ  "Việc sử dụng màu sắc liên quan đến công đoạn nào của mô phỏng", rect + fillcolor của canvas thứ 2 + text
                
                        /* Màu xanh lá nhẹ cây là đỉnh nguồn
                           Màu xám nhạt là đỉnh đã viếng thăm
                           Màu đỏ của cạnh là cạnh đang xét
                           Màu đỏ của đỉnh là đỉnh đang xét
                           Màu vàng nhạt là đỉnh đang được đo khoảng cách với đỉnh "màu đỏ" đang xét
                        */
};